...
def s_test(x_test, m_test, y_test, model, i, samples_loader, gpu=-1, damp=0.01, scale=25.0, loss_func="cross_entropy",
           clip_value=100):
    """s_test can be precomputed for each test point of interest, and then
    multiplied with grad_z to get the desired value for each training point.
    Here, stochastic estimation is used to calculate s_test. s_test is the
    Inverse Hessian Vector Product.
    https://arxiv.org/pdf/1611.03777.pdf description about [20].
    Arguments:
    ...
    v = grad_z(x_test, m_test, y_test, model, gpu, loss_func=loss_func)
    h_estimate = v
    ...
    for i, (did, tid, x_train, m_train, y_train) in enumerate(progress_bar):

        if gpu >= 0:
            x_train, m_train, y_train = x_train.cuda(), m_train.cuda(), y_train.cuda()

        def f(*new_params):
            load_weights(model, names, new_params);
            #            pdb.set_trace()
            out = model(x_train, attention_mask=m_train)
        ...
        with torch.no_grad():
            hv = [torch.clamp(hhv, -clip_value, clip_value) for hhv in hv]
            h_estimate = [
...
def calc_loss(outputs, labels, loss_func="cross_entropy"):
    """Calculates the loss
...
def grad_z(input_ids, token_type_ids, lm_labels, model):
    ...
    # loss = loss_fct(loss, lm_labels)

    #not_ignore = shift_labels.ne(pad_id)
    # not_ignore = lm_labels.ne(pad_id)
    num_targets = shift_labels.ne(pad_id).long().sum().item()

    loss = loss / num_targets
    return loss


# def grad_z(x, m, y, model, gpu=-1):
...
#     # x.to(args.device), token_type_ids.to(args.device), lm_labels.to(args.device)
#
#     outputs = model(
#         input_ids=x,
#         token_type_ids=m,
...
def s_test_sample(
    ...
    for i in range(r):
        ...
        cur_estimate = s_test(
            x_test, m_test, y_test, model, i, hessian_loader, gpu=gpu, damp=damp, scale=scale, loss_func=loss_func,
        )
        with torch.no_grad():
            inverse_hvp = [
